...
- optionally running sync/test/lint/typecheck and a local build
...
Options:
  ...
  --skip-verify      Skip uv sync/test/lint/typecheck/build verification
...
Examples:
  ./scripts/release.sh 0.6.5
  ./scripts/release.sh 0.6.5 --no-auto-merge
EOF
}
...
if [ "$skip_verify" -eq 0 ]; then
  ...
  uv sync --python 3.12 --extra dev --group dev
  uv run --python 3.12 python -m pytest -v --tb=short
