...
- Traces are rebuilt from their root observation: `name`, `tags` and `release` from the trace context; `input`, `output`, `metadata` and `latency` from the root; `html_path` from the project id. `fetch_trace` also sums `total_cost` over the trace's observations and reads `scores` from Scores API v3 when the SDK has it (4.8.1+). `fetch_traces` no longer returns observation ids, scores or total cost; observations still come with `include_observations=True`.
- `fetch_sessions` groups the window's root observations by session: `id`, `last_trace_at`, `user_id`, `environment`, `project_id`. v4 has no session object, so `created_at` is gone.
...
- v2 rows get the snake_case keys the v1 SDK models produced, and `input`/`output` strings are parsed back to JSON where they are JSON. Metadata values over 200 characters come back truncated (the v2 default), except the keys the route-decision tools read.
