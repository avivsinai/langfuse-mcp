...
- Dataset-run reads (`list_dataset_runs`, `get_dataset_run`, `list_dataset_run_items`) use the experiments and experiment-items APIs when the SDK exposes them (Python SDK 4.13.1+). Dataset names resolve through `datasets.get`; experiment requests start at the dataset's `created_at`, or 2000-01-01 UTC when it is missing. `page` walks the cursor with the existing page and scan caps. HTTP 404/405 falls back to legacy reads; other errors propagate. If an older SDK has no experiments API and legacy reads are gone, `ERR_LANGFUSE_EXPERIMENTS_SDK_UPGRADE` asks for Langfuse 4.13.1+.
- **Breaking output change:** Experiment run lists retain `id`, `name`, `description`, `dataset_id`, `dataset_name`, and `metadata`, but lose legacy `created_at` and `updated_at`; experiments return `start_time`, `end_time`, `item_count`, and scores (up to 50 per record). An experiment `id` is not guaranteed to match a legacy run ID. `total` is null on cursor pages, and `next_page` indicates more rows. `get_dataset_run` keeps `dataset_run_items` (and the existing `items` alias), but loses the legacy `deprecation` field. Each item keeps `trace_id`; `dataset_run_id`, `dataset_run_name`, and `dataset_item_id` come from experiment fields. Item `created_at` and `updated_at` are absent (`start_time` and `end_time` describe experiment activity), and legacy `observation_id` is unavailable. Item `id` comes from the experiment endpoint and is not guaranteed to match the old run-item ID. Item `metadata` is the experiment API's metadata field and may differ from old link metadata; items also expose `input`, `output`, `expected_output`, and scores. Additional `dataset_id`, `run_name`, `run_description`, and `dataset_version` aliases come from corresponding experiment fields.
- `create_dataset_run_item` keeps the old POST route while available. Once removed, `ERR_LANGFUSE_RUN_ITEM_CREATE_REMOVED` explains that an existing trace cannot be linked through an API; use the SDK experiment runner or OTel ingestion. `delete_dataset_run` keeps the old DELETE route while available. After removal, `delete_traces=True` is required to delete the run's traces in batches of at most 1,000; this also deletes their observations and related scores. Without opt-in, `ERR_LANGFUSE_RUN_DELETE_REQUIRES_TRACES` leaves data intact. The replacement reports `deleted_trace_count` and cannot delete an experiment record alone.
